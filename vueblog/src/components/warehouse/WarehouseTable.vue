...
<template>
  <div>
    ...
    <div class="blog_table_footer">
      <el-button type="danger" size="mini" style="margin: 0px;" v-show="this.warehouse!=null && this.warehouse.length>0"
      ...
      <el-pagination
        ...
        :total="totalCount" @current-change="currentChange" v-show="this.warehouse!=null && this.warehouse.length>0">
      </el-pagination>
    </div>
  </div>
</template>
...
<script>
    ...
    export default{
        ...
        mounted: function () {
            var _this = this;
            this.loading = true;
            this.loadWarehouse(1, this.pageSize);
            var _this = this;
            window.bus.$on('blogTableReload', function () {
                _this.loading = true;
                _this.loadWarehouse(_this.currentPage, _this.pageSize);
            })
        },
        methods: {
            searchClick(){
            ...
            itemClick(row){
                this.$router.push({path: '/addWarehouse', query: {from: "query",id:row.id}})
            },
            deleteMany(){
            ...
            currentChange(currentPage){
                ...
                this.loading = true;
                this.loadWarehouse(currentPage, this.pageSize);
            },
            loadWarehouse(page, count){
                var _this = this;
                var url = '';
                if (this.state == 3) {
                    url = "/warehouse/getAll" + "?page=" + page + "&count=" + count + "&keywords=" + this.keywords;
                }
                getRequest(url).then(resp=> {
                    _this.loading = false;
                    if (resp.status == 200) {
                        _this.success = true;
                        _this.warehouse = resp.data.warehouse;
            ...
            handleEdit(index, row) {
                this.$router.push({path: '/addWarehouse', query: {from: this.activeName, id: row.id}});
            },
