...
<!--      <el-button type="primary" icon="icon-zhyqcheck" size="mini" style="margin-left: 3px;"  @click="check">审核</el-button>-->
    ...
    <add-out-area v-if="showAdd" ref="addOutArea" :state="states" :id="id"></add-out-area>
    <el-table
      ref="multipleTable"
      :data="outArea"
      tooltip-effect="dark"
    ...
    <div class="blog_table_footer">
      <el-button type="danger" size="mini" style="margin: 0px;" v-show="this.outArea!=null && this.outArea.length>0"
      ...
      <el-pagination
        ...
        :total="totalCount" @current-change="currentChange" v-show="this.outArea!=null && this.outArea.length>0">
      </el-pagination>
    </div>
  </div>
</template>
...
<script>
    ...
    import addOutArea from './AddOutArea'
    ...
    export default{
        ...
        mounted: function () {
            var _this = this;
            this.loading = true;
            this.loadOutArea(1, this.pageSize);
            window.bus.$on('blogTableReload', function () {
                _this.loading = true;
                _this.loadOutArea(_this.currentPage, _this.pageSize);
            })
        },
        methods: {
            searchClick(){
            ...
            currentChange(currentPage){
                ...
                this.loading = true;
                this.loadOutArea(currentPage, this.pageSize);
            },
            loadOutArea(page, count){
                var _this = this;
                var url = '';
                if (this.state == 1) {
                    url = "/outArea/getAll" + "?page=" + page + "&count=" + count + "&keywords=" + this.keywords;
                }
                getRequest(url).then(resp=> {
                    _this.loading = false;
                    if (resp.status == 200) {
                        _this.success = true;
                        _this.outArea = resp.data.outArea;
