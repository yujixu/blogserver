...
<template>
  <div>
    ...
    <div class="blog_table_footer">
      <el-button type="danger" size="mini" style="margin: 0px;" v-show="this.supplier!=null && this.supplier.length>0"
      ...
      <el-pagination
        ...
        :total="totalCount" @current-change="currentChange" v-show="this.supplier!=null && this.supplier.length>0">
      </el-pagination>
    </div>
  </div>
</template>
...
<script>
    ...
    export default{
        ...
        mounted: function () {
            var _this = this;
            this.loading = true;
            this.loadSupplier(1, this.pageSize);
            var _this = this;
            window.bus.$on('blogTableReload', function () {
                _this.loading = true;
                _this.loadSupplier(_this.currentPage, _this.pageSize);
            })
        },
        methods: {
            searchClick(){
            ...
            itemClick(row){
                this.$router.push({path: '/addSupplier', query: {from: "query",id:row.id}})
            },
            deleteMany(){
            ...
            currentChange(currentPage){
                ...
                this.loading = true;
                this.loadSupplier(currentPage, this.pageSize);
            },
            loadSupplier(page, count){
                var _this = this;
                var url = '';
                if (this.state == 5) {
                    url = "/supplier/getAll" + "?page=" + page + "&count=" + count + "&keywords=" + this.keywords;
                }
            ...
            handleEdit(index, row) {
                this.$router.push({path: '/addSupplier', query: {from: this.activeName, id: row.id}});
            },
