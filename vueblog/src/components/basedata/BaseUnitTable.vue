...
<template>
  <div>
    ...
    <div class="blog_table_footer">
      <el-button type="danger" size="mini" style="margin: 0px;" v-show="this.baseUnit!=null && this.baseUnit.length>0"
      ...
      <el-pagination
        ...
        :total="totalCount" @current-change="currentChange" v-show="this.baseUnit!=null && this.baseUnit.length>0">
      </el-pagination>
    </div>
  </div>
</template>
...
<script>
    ...
    export default{
        ...
        mounted: function () {
            var _this = this;
            this.loading = true;
            this.loadBaseUnit(1, this.pageSize);
            var _this = this;
            window.bus.$on('blogTableReload', function () {
                _this.loading = true;
                _this.loadBaseUnit(_this.currentPage, _this.pageSize);
            })
        },
        methods: {
            searchClick(){
            ...
            add(){
                this.$router.push({path: '/addBaseUnit'})
            },
            itemClick(row){
                this.$router.push({path: '/addBaseUnit', query: {from: "query",id:row.id}})
            },
            deleteMany(){
            ...
            currentChange(currentPage){
                ...
                this.loading = true;
                this.loadBaseUnit(currentPage, this.pageSize);
            },
            loadBaseUnit(page, count){
                var _this = this;
                var url = '';
                if (this.state == 3) {
                    url = "/baseUnit/getAll" + "?page=" + page + "&count=" + count + "&keywords=" + this.keywords;
                }
                getRequest(url).then(resp=> {
                    _this.loading = false;
                    if (resp.status == 200) {
                        _this.success = true;
                        _this.baseUnit = resp.data.baseUnit;
            ...
            handleEdit(index, row) {
                this.$router.push({path: '/addBaseUnit', query: {from: this.activeName, id: row.id}});
            },
            ...
            delete(){
                var _this = this;
                this.$confirm('永久删除该计量单位, 是否继续?', '提示', {
