...
<template>
  <div>
    ...
    <div class="blog_table_footer">
      <el-button type="danger" size="mini" style="margin: 0px;" v-show="this.material!=null && this.material.length>0"
      ...
      <el-pagination
        ...
        :total="totalCount" @current-change="currentChange" v-show="this.material!=null && this.material.length>0">
      </el-pagination>
    </div>
  </div>
</template>
...
<script>
    ...
    export default{
        ...
        mounted: function () {
            var _this = this;
            this.loading = true;
            this.loadMaterial(1, this.pageSize);
            window.bus.$on('blogTableReload', function () {
                _this.loading = true;
                _this.loadMaterial(_this.currentPage, _this.pageSize);
            })
        },
        methods: {
            searchClick(){
            ...
            itemClick(row){
                this.$router.push({path: '/addMaterial', query: {from: "query",id:row.id}})
            },
            deleteMany(){
            ...
            currentChange(currentPage){
                ...
                this.loading = true;
                this.loadMaterial(currentPage, this.pageSize);
            },
            loadMaterial(page, count){
                var _this = this;
                var url = '';
                if (this.state == -1) {
                    url = "/material/getAll" + "?page=" + page + "&count=" + count + "&keywords=" + this.keywords;
                }
                getRequest(url).then(resp=> {
                    _this.loading = false;
                    if (resp.status == 200) {
                        _this.success = true;
                        _this.material = resp.data.material;
            ...
            handleEdit(index, row) {
                this.$router.push({path: '/addMaterial', query: {from: this.activeName, id: row.id}});
            },
