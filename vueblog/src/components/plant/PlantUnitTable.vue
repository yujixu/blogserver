...
<template>
  <div>
    ...
    <div class="blog_table_footer">
      <el-button type="danger" size="mini" style="margin: 0px;" v-show="this.unit!=null && this.unit.length>0"
      ...
      <el-pagination
        ...
        :total="totalCount" @current-change="currentChange" v-show="this.unit!=null && this.unit.length>0">
      </el-pagination>
    </div>
  </div>
</template>
...
<script>
    ...
    export default{
        ...
        mounted: function () {
            var _this = this;
            this.loading = true;
            this.loadUnit(1, this.pageSize);
            var _this = this;
            window.bus.$on('blogTableReload', function () {
                _this.loading = true;
                _this.loadUnit(_this.currentPage, _this.pageSize);
            })
        },
        methods: {
            searchClick(){
            ...
            add(){
                this.$router.push({path: '/addPlantUnit'})
            },
            itemClick(row){
                this.$router.push({path: '/addPlantUnit', query: {from: "query",id:row.id}})
            },
            deleteMany(){
            ...
            currentChange(currentPage){
                ...
                this.loading = true;
                this.loadUnit(currentPage, this.pageSize);
            },
            loadUnit(page, count){
                var _this = this;
                var url = "/plantUnit/getAll" + "?page=" + page + "&count=" + count + "&keywords=" + this.keywords;
                getRequest(url).then(resp=> {
                    _this.loading = false;
                    if (resp.status == 200) {
                        _this.success = true;
                        _this.unit = resp.data.unit;
            ...
            handleEdit(index, row) {
                this.$router.push({path: '/addPlantUnit', query: {from: this.activeName, id: row.id}});
            },
            ...
            delete(){
                var _this = this;
                this.$confirm('永久删除该植物类型, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    _this.loading = true;
                    var url = "/plantUnit/delete";
