...
<template>
  <div>
    ...
    <div class="blog_table_footer">
      <el-button type="danger" size="mini" style="margin: 0px;" v-show="this.type!=null && this.type.length>0"
      ...
      <el-pagination
        ...
        :total="totalCount" @current-change="currentChange" v-show="this.type!=null && this.type.length>0">
      </el-pagination>
    </div>
  </div>
</template>
...
<script>
    ...
    export default{
        ...
        mounted: function () {
            var _this = this;
            this.loading = true;
            this.loadType(1, this.pageSize);
            window.bus.$on('blogTableReload', function () {
                _this.loading = true;
                _this.loadType(_this.currentPage, _this.pageSize);
            })
        },
        methods: {
            searchClick(){
            ...
            add(){
                this.$router.push({path: '/addPlantType'})
            },
            itemClick(row){
                this.$router.push({path: '/addPlantType', query: {from: "query",id:row.id}})
            },
            deleteMany(){
            ...
            currentChange(currentPage){
                ...
                this.loading = true;
                this.loadType(currentPage, this.pageSize);
            },
            loadType(page, count){
                var _this = this;
                var url = "/plantType/getAll" + "?page=" + page + "&count=" + count + "&keywords=" + this.keywords;
            ...
            handleEdit(index, row) {
                this.$router.push({path: '/addPlantType', query: {from: this.activeName, id: row.id}});
            },
            ...
            delete(){
                var _this = this;
                this.$confirm('永久删除该植物种类, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    _this.loading = true;
                    var url = "/plantType/delete";
